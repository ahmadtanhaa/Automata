...
#
# Purpose:
#   Create timestamped MongoDB and MySQL backups for a live environment,
#   transfer them to a remote backup server, and clean local temporary backups.
#
# Notes:
#   - Sanitized version for public portfolio use.
#   - Do not hard-code credentials in production.
#   - Use environment variables, secret managers, or protected configuration files.
#
# License:
#   Apache License 2.0
...
REMOTE_LIVE_MONGO_DIR="${REMOTE_LIVE_MONGO_DIR:-/var/log/db_backup/Live/Mongodb}"
REMOTE_LIVE_MYSQL_DIR="${REMOTE_LIVE_MYSQL_DIR:-/var/log/db_backup/Live/Mysql}"
...
log "Starting MongoDB live backup..."
...
log "Transferring MongoDB backup to remote server..."

scp -r -P "$REMOTE_PORT" \
  "$MONGO_OUTPUT_DIR" \
  "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_LIVE_MONGO_DIR}/"

log "MongoDB backup transferred successfully."

# -----------------------------
# MySQL backup
# -----------------------------

log "Starting MySQL live backup..."
...
log "Transferring MySQL backups to remote server..."

scp -r -P "$REMOTE_PORT" \
  "$MYSQL_OUTPUT_DIR" \
  "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_LIVE_MYSQL_DIR}/"

log "MySQL backups transferred successfully."
...
log "Live backup process completed successfully."
